...
int DemoProfilePositionMode(HANDLE p_DeviceHandle, unsigned short p_usNodeId, unsigned int & p_rlErrorCode)
{
	...
	else
	{
		...
		//positionList.push_back(-1691);
		//positionList.push_back(-3382);
		//positionList.push_back(-6226);
		//positionList.push_back(-10839);
		positionList.push_back(-13686);
		//positionList.push_back(-6226);
		positionList.push_back(0);
		...
		for(list<long>::iterator it = positionList.begin(); it !=positionList.end(); it++)
		{
			...
			int timeout = 6000;
			VCS_WaitForTargetReached(p_DeviceHandle, p_usNodeId, timeout, &p_rlErrorCode);
