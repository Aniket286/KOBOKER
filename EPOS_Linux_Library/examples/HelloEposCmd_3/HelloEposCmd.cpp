...
void PrintUsage()
{
	...
	cout << "\t-b   : baudrate (125000 (for can), 115200, 1000000,... default - 1000000)" << endl;
...
int DemoProfilePositionMode(HANDLE p_DeviceHandle, unsigned short p_usNodeId, unsigned int & p_rlErrorCode)
{
	...
	else
	{
		...
		positionList.push_back(-3000);
		//positionList.push_back(-100000);
		//positionList.push_back(0000);
		//positionList.push_back(0000);
		//positionList.push_back(0000);
...
bool DemoProfileVelocityMode(HANDLE p_DeviceHandle, unsigned short p_usNodeId, unsigned int & p_rlErrorCode)
{
	...
	//p_usNodeId=15;
	if((VCS_ActivateProfileVelocityMode(p_DeviceHandle, 15, &p_rlErrorCode) == 0)||(VCS_ActivateProfileVelocityMode(p_DeviceHandle, 14, &p_rlErrorCode) == 0)||(VCS_ActivateProfileVelocityMode(p_DeviceHandle, 13, &p_rlErrorCode) == 0)||(VCS_ActivateProfileVelocityMode(p_DeviceHandle, 12, &p_rlErrorCode) == 0)||(VCS_ActivateProfileVelocityMode(p_DeviceHandle, 11, &p_rlErrorCode) == 0)||(VCS_ActivateProfileVelocityMode(p_DeviceHandle, 10, &p_rlErrorCode) == 0))
	{
	...
	LogInfo(msg.str());
		...
		for(list<long>::iterator it = velocityList.begin(); it !=velocityList.end(); it++)
		{
			...
			VCS_MoveWithVelocity(p_DeviceHandle, 15, targetvelocity, &p_rlErrorCode);
				VCS_MoveWithVelocity(p_DeviceHandle, 14, targetvelocity, &p_rlErrorCode);
				VCS_MoveWithVelocity(p_DeviceHandle, 13, targetvelocity, &p_rlErrorCode);
				VCS_MoveWithVelocity(p_DeviceHandle, 12, targetvelocity, &p_rlErrorCode);
				VCS_MoveWithVelocity(p_DeviceHandle, 11, targetvelocity, &p_rlErrorCode);
	stringstream msg;

	msg << "Velocity mode" << p_usNodeId;

	LogInfo(msg.str());


			if((VCS_MoveWithVelocity(p_DeviceHandle, 15, targetvelocity, &p_rlErrorCode) == 0)||(VCS_MoveWithVelocity(p_DeviceHandle, 14, targetvelocity, &p_rlErrorCode) == 0)||(VCS_MoveWithVelocity(p_DeviceHandle, 13, targetvelocity, &p_rlErrorCode) == 0)||(VCS_MoveWithVelocity(p_DeviceHandle, 12, targetvelocity, &p_rlErrorCode) == 0)||(VCS_MoveWithVelocity(p_DeviceHandle, 11, targetvelocity, &p_rlErrorCode) == 0)||(VCS_MoveWithVelocity(p_DeviceHandle, 10, targetvelocity, &p_rlErrorCode) == 0))
			{
		...
		if(lResult == MMC_SUCCESS)
		{
			...
			if((VCS_HaltVelocityMovement(p_DeviceHandle, 15, &p_rlErrorCode) == 0) || (VCS_HaltVelocityMovement(p_DeviceHandle, 14, &p_rlErrorCode) == 0) || (VCS_HaltVelocityMovement(p_DeviceHandle, 13, &p_rlErrorCode) == 0) || (VCS_HaltVelocityMovement(p_DeviceHandle, 12, &p_rlErrorCode) == 0) || (VCS_HaltVelocityMovement(p_DeviceHandle, 11, &p_rlErrorCode) == 0) || (VCS_HaltVelocityMovement(p_DeviceHandle, 10, &p_rlErrorCode) == 0))
			{
				lResult = MMC_FAILED;
